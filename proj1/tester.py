...
occupation_list = ['-','accommodation','warden','accounting','technician','accounts','clerk','-','bookkeeper','acoustics','consultant',
                   ...
                   'youth','and','community','worker','youth','offending','team','officer','zookeeper','zoologist']
...
def find_tile_in_training_set(training_set):
    return 0
...
if __name__ == '__main__':
    ...
    path_to_classifer = sys.argv[2]
    path_to_result = sys.argv[3]
    with open(path_to_classifer,'rb') as f:
        (clf,vec) = pickle.load(f)
    test_set = reading_data(path_to_testing_data)
    temp_test_feature_vector_dict = convert_word_to_feature_dict(test_set)
    test_feature_vector_dict = vec.transform(temp_test_feature_vector_dict).toarray()
    prediction = clf.predict(test_feature_vector_dict)
    result = []
    count = 0
    for sen_index in range(len(test_set)):
        result.append([])
        for word in test_set[sen_index]:
            temp = (word[0],prediction[count])
            result[sen_index].append(temp)
            count += 1
